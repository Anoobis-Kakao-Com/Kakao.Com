...
// 데스크탑 폴링(/api/geocode-latest)이 읽어갈 저장 키.
// 위치 자체가 "최근 모바일이 보낸 값 1개"만 있으면 되는 용도이므로 고정 키 하나만 사용한다.
...
async function saveLatestAddress(payload) {
    try {
        ...
        // 저장 실패는 모바일 응답 자체에는 영향을 주지 않는다 (베스트 에포트 저장).
        console.error("Address.js: Blobs 저장 실패", e && e.message);
    }
}
...
export default async function handler(request) {
    ...
    try {
        ...
        if (!data.documents || data.documents.length === 0) {
            const emptyPayload = { address: null, message: '주소를 찾을 수 없습니다.' };
            await saveLatestAddress(emptyPayload);
            return new Response(JSON.stringify(emptyPayload), { status: 200, headers: HEADERS });
        }
        ...
        if (roadAddress) {
            ...
            await saveLatestAddress(payload);
