...
const BLOB_STORE_NAME = 'location-cache';
const BLOB_KEY = 'geocode-latest';

// 응답 본문을 만들면서 동시에 최신 결과를 Netlify Blobs 에 저장한다.
// 저장에 실패하더라도(예: Blobs 미지원 환경) 원래 응답 흐름에는 영향을 주지 않는다.
async function respondAndStore(bodyObj, status, headers) {
    try {
        ...
        // Blobs 저장 실패는 무시하고 정상 응답은 그대로 내려준다.
    }
...
export default async function handler(request) {
    ...
    // 저장된 값만 조회하는 전용 엔드포인트 (외부 API 호출 없음)
    if (url0.pathname === '/api/geocode-latest') {
        const HEADERS0 = { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' };
        ...
        try {
            ...
            if (!saved) {
                return new Response(JSON.stringify({ data: null }), { status: 200, headers: HEADERS0 });
            }
            return new Response(JSON.stringify({ data: saved.data, savedAt: saved.savedAt }), { status: 200, headers: HEADERS0 });
        } catch (e) {
            return new Response(JSON.stringify({ data: null, error: '저장소 조회 실패', detail: e.message }), { status: 200, headers: HEADERS0 });
        }
    }
